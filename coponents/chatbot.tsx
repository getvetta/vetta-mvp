...
export default function ChatbotPage() {
  ...
  // Load custom questions (and append to defaults)
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    const updated: QA[] = [
      ...responses,
      { question: activeQuestions[step], answer: input.trim() },
    ];
    ...
    try {
      ...
      if (!riskRes.ok) {
        const text = await riskRes.text().catch(() => '');
        throw new Error(text || 'Risk analysis failed');
      }

      const riskData = await riskRes.json();
      const risk: Risk = (riskData.risk as Risk) ?? 'medium';
      const reasoning: string = riskData.reasoning ?? 'No reasoning provided.';
      setRiskResult({ risk, reasoning });

      // Save to Supabase (client insert). If RLS blocks, switch to a server route that uses service role.
      const { error } = await supabase.from('assessments').insert([
        {
          dealer_id: userId || 'unknown',
          answers: updated,
          risk,
          reasoning,
        },
      ...
      if (error) {
        // Show message but still show result to user
        setErrorMsg(
          'Saved locally, but storing securely failed. The dealer may not see this until it’s retried.'
        );
      }
