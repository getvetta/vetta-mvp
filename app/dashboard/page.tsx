...
export default function DashboardPage() {
  ...
  if (loading)
    return (
      <p className="p-6 text-center text-gray-700 dark:text-gray-300">Loading...</p>
    );
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 sm:p-6 lg:p-8 text-gray-900 dark:text-white transition-colors">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          ...
          <div
            key={label}
            className="bg-white dark:bg-gray-800 p-5 rounded-lg shadow flex flex-col items-center"
          >
      ...
      <section className="bg-white dark:bg-gray-800 p-5 rounded-lg shadow mb-6">
        ...
        <p className="text-gray-700 dark:text-gray-300">
          {data.coaching || 'No insights available yet.'}
        </p>
      </section>
      ...
      <section className="bg-white dark:bg-gray-800 p-5 rounded-lg shadow mb-6">
        ...
        >
          ...
          <button
            ...
            className="text-white rounded-r px-5 py-2 hover:brightness-90 active:scale-95 transition-transform"
          >
            Add
          </button>
        </form>
        <ul className="divide-y divide-gray-200 dark:divide-gray-700 max-h-60 overflow-auto">
          {questions.length === 0 && (
            <li className="text-gray-500 dark:text-gray-400 py-2">No custom questions.</li>
          )}
          {questions.map((q) => (
            <li
              key={q.id}
      ...
      {/* Branding & Settings */}
      <section className="bg-white dark:bg-gray-800 p-5 rounded-lg shadow mb-6">
        <h2 className="text-xl font-semibold mb-3">Branding & Settings</h2>
        <form onSubmit={updateBranding} className="flex flex-col gap-4 max-w-md">
          <div>
            <label htmlFor="logo_url" className="block mb-1 font-medium">
              Logo URL
            </label>
            <input
              id="logo_url"
          ...
          <div>
            <label htmlFor="theme_color" className="block mb-1 font-medium">
              Theme Color
            </label>
            <input
              id="theme_color"
          ...
          <button
            type="submit"
            style={{ backgroundColor: data.theme_color }}
            className="text-white px-5 py-2 rounded hover:brightness-90 active:scale-95 transition-transform"
          >
            Save
          </button>
        </form>
